...
class HotspotService
{
        ...
        StatusMessage $status): bool
    {
        ...
        $baseIface = validateInterface($post_data['interface']) ? $post_data['interface'] : OPENAP_WIFI_AP_INTERFACE;
        ...
        // Persist derived state only after the complete request is valid.
        $this->hostapd->persistHostapdIni($states, $apIface, $cliIface, $arrHostapdConf);
        $_SESSION['ap_interface'] = $sessionIface;

        try {
            $isRepeaterContainer = defined('OPENAP_REPEATER_CONTAINER') && OPENAP_REPEATER_CONTAINER;
            $isEthernetBridge = false;
            if ($isRepeaterContainer && is_readable('/etc/openap/repeater.ini')) {
                $repeaterProfile = parse_ini_file('/etc/openap/repeater.ini', true);
                $isEthernetBridge = ($repeaterProfile['mode']['current'] ?? '') === 'ap_ethernet_bridge';
            }

            // normalize state flags
            $validated['interface'] = $apIface;
            $validated["bridgeName"] = (!empty($states["BridgedEnable"]) || $isEthernetBridge)
                ? "br0"
                : null;
            $validated['bridge']    = !empty($states['BridgedEnable']) || $isEthernetBridge;
            ...
            $validated['txpower']   = $post_data['txpower'] ?? null;
            ...
            // hostapd
            $isSkynetExistingAp = (defined('OPENAP_RASPBERRY_NATIVE') && OPENAP_RASPBERRY_NATIVE)
                || (defined('OPENAP_SKYNET_EXISTING_AP') && OPENAP_SKYNET_EXISTING_AP);
            $isHostapdOnlyProfile = $isSkynetExistingAp || $isRepeaterContainer;
            $config = $this->hostapd->buildConfig($validated, $status);
            $this->hostapd->saveConfig($config, $dualAPEnable, $validated['interface'], $isHostapdOnlyProfile);
            $this->maybeSetRegDomain($validated['country_code'], $status);

            $status->addMessage('WiFi hotspot settings saved.', 'success');

            if ($isHostapdOnlyProfile) {
                $profileName = $isRepeaterContainer ? 'OpenAP mode' : 'Skynet mode';
                $status->addMessage($profileName . ': hostapd was restarted; DHCP, dhcpcd and network routing were left unchanged.', 'info');
                if ($isRepeaterContainer) {
                    $status->addMessage('If the Wi-Fi band or channel changed, reconnect clients to the hotspot.', 'info');
                }
                return true;
            }
            ...
            // dnsmasq
            try {
                $syscfg = $this->dnsmasq->getConfig($validated['interface'] ?? OPENAP_WIFI_AP_INTERFACE);
     ...
     * @param StatusMessage $status
     * @param int|null      $expectedChannel
     * @return bool
     */
    public function maybeSetTxPower(
        string $iface,
        $dbm,
        StatusMessage $status,
        ?int $expectedChannel = null
    ): bool
    {
        ...
        $txpower = $dbm === 'auto' ? 'auto' : (string)(int)$dbm;
        $descriptors = [
            0 => ['pipe', 'r'],
            1 => ['pipe', 'w'],
            2 => ['pipe', 'w'],
        ...
        $process = proc_open(
            ['sudo', '/usr/local/sbin/openap-set-txpower'],
            $descriptors,
            $pipes
        );
        if (!is_resource($process)) {
            $status->addMessage('Unable to start the transmit-power helper.', 'danger');
            return false;
        }
        ...
        fwrite(
            ...
            $iface . PHP_EOL
            . $txpower . PHP_EOL
            . ($expectedChannel !== null ? (string) $expectedChannel : '') . PHP_EOL
        );
        fclose($pipes[0]);
        ...
        if ($return !== 0) {
            $status->addMessage(
                $error !== '' ? 'Unable to set transmit power: ' . $error : 'Unable to set transmit power.',
                'danger'
            );
            return false;
        }

        $message = $txpower === 'auto'
            ? 'Transmit power set to auto.'
            : 'Transmit power set to ' . $txpower . ' dBm.';
        $status->addMessage($message, 'success');
        return true;
    }
