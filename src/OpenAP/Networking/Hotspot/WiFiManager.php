...
class WiFiManager
{
    ...
    public function knownWifiStations(&$networks)
    {
        ...
        exec(' sudo cat ' . OPENAP_WPA_SUPPLICANT_CONFIG, $known_return);
    ...
    {
        $cacheTime = filemtime(OPENAP_WPA_SUPPLICANT_CONFIG);
     ...
     *
     * Native OpenAP profiles resolve interface roles from repeater.ini through
     * the OPENAP_WIFI_* constants. The legacy hostapd.ini file must not
     * override those roles after an installer reinstall or AP-radio change.
     */
    public function getWifiInterface()
    {
        ...
        $nativeRoleProfile = defined('OPENAP_REPEATER_CONTAINER') && OPENAP_REPEATER_CONTAINER;
        $iface = $nativeRoleProfile
            ? OPENAP_WIFI_AP_INTERFACE
            : ($arrHostapdConf['WifiInterface'] ?? OPENAP_WIFI_AP_INTERFACE);
        $_SESSION['ap_interface'] = $iface;

        if (!validateInterface($iface)) {
            $iface = OPENAP_WIFI_AP_INTERFACE;
        }

        // @TODO Get client interface from persisted value
        $client_iface = $nativeRoleProfile ? OPENAP_WIFI_CLIENT_INTERFACE : '';
        if ($nativeRoleProfile) {
            $_SESSION['wifi_client_interface'] = $client_iface;
        }
        ...
        if ($_SESSION['wifi_client_interface'] === null && !isset($_POST['wifiClientInterface'])) {
            exec("/usr/sbin/iw dev | awk '$1==\"Interface\" && $2!=\"$iface\" {print $2}'", $iface2);
...
CONF;
    ...
    public function writeWpaSupplicant(array $networks, string $interface): array
    {
        ...
        fwrite($wpa_file, 'ctrl_interface=DIR=' . OPENAP_WPA_CTRL_INTERFACE . ' GROUP=netdev' . PHP_EOL);
        ...
        if ($ok) {
            system('sudo cp /tmp/wifidata ' . OPENAP_WPA_SUPPLICANT_CONFIG, $returnval);
            if ($returnval == 0) {
