...
class Dashboard {
    ...
    public function __construct() {
        $this->firewallConfig = OPENAP_CONFIG.'/networking/firewall.conf';
    }
    ...
    public function getWirelessDetails(string $interface): array
    {
        ...
        if ($ssid === '-') {
            $ssid = $this->getConfiguredApSsid($interface);
        }
    ...
    private function getConfiguredApSsid(string $interface): string
    {
        if (!defined('OPENAP_WIFI_AP_INTERFACE') || $interface !== OPENAP_WIFI_AP_INTERFACE) {
            return '-';
        }

        $config = defined('OPENAP_HOSTAPD_CONFIG')
        ...
        foreach ($lines as $line) {
            if (str_starts_with($line, 'ssid=')) {
                $ssid = trim(substr($line, strlen('ssid=')));
    ...
    {
        return preg_match('/^[[:space:]]*ssid[[:space:]]+(.+)$/mi', $output, $matches)
            ? trim($matches[1])
            : '-';
    }
    ...
    public function getEthernetClients(): int
    {
        ...
        $leaseFile = OPENAP_DNSMASQ_LEASES;
